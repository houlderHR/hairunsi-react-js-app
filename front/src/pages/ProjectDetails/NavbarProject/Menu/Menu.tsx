...
const Menu: FC<{ id: string }> = ({ id }) => {
  ...
  const onclikedMenu=()=>{
    setIsDropdown((preview)=>!preview)
  }
  return <nav className="md:pl-[116px] transition ease-in-out duration-500">
    <div className='absolute z-[1000] hover:cursor-pointer right-2 top-3 md:hidden' onClick={onclikedMenu}>
      <Icon name='bars-solid' size={20}/>
    </div>
      {isDropdown && <ul className="absolute  transition shadow-bottom bg-white w-full  px-4 border-b-1 pb-5 flex md:hidden flex-col text-gray-4 text-xs font-bold leading-[18px] list-none">
        <NavLink
          to={`${routes.authentified.subpaths.project.path}/${id}/${routes.authentified.subpaths.project.subpaths.id.subpaths.main_info.path}`}
          className={({ isActive }) => (isActive ? menu.active +'  px-2 py-2 !border-none text-secondary hover:bg-white-1' : menu.inactive +' hover:bg-white-1 px-2 py-2')}
        >
        ...
        <NavLink
          ...
          className={({ isActive }) => (isActive ? menu.active+' hover:bg-white-1 px-2 py-2 !border-b-none !border-none text-secondary' : menu.inactive+' hover:bg-white-1 px-2 py-2')}
        >
